...
#`Record` that contains the `sender`, `receiver`, and `amount` values. The
# updated `Block`should also include the attributes for `creator_id` and `prev_hash`.
...
if st.button("Add Block"):
    ...
    # and `amount` values
    new_block = Block(
        record = Record(sender = input_sender, receiver = input_receiver, 
            amount = input_amount),
        creator_id=42,
